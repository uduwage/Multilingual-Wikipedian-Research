...
IS_GLOBAL_USER_FILES = os.path.expanduser('~/outputs/data_10_10/')
...
class GlobalUserIntersect():
	...
	def is_user_global(self):
		completedfile = []
		with open(os.path.expanduser(OUT_DIR + 'active_users.txt'), 'w+') as data_file:
			writer = csv.writer(data_file)
			writer.writerow(('wiki', 'match', 'unmatch', 'unmatch_active', 'match_active'))
			for filename in os.listdir(IS_GLOBAL_USER_FILES):
				#import pdb
				#pdb.set_trace()
				logging.info("reading users from " + filename)
				...
				if filename not in completedfile:
					...
					writer.writerow([filename.split("-")[0], match, unmatch, unmatch_active, match_active])
...
def main():
	...
	globalusers = GlobalUserIntersect('en', '')
