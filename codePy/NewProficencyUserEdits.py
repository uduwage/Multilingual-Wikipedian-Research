...
class EditPatternUserProficency():
	...
	def __init__(self):
		self.dir  = os.path.expanduser('~/outputs/All_Wiki_Global_Users/')#'~/outputs/Global_Unique/'
		self.outdir = os.path.expanduser('~/outputs/namespace/')#Global_Edit_Count/
	...
	def user_namespace_contribution(self):
		...
		for filename in os.listdir(self.dir):
			...
			with open(os.path.expanduser(self.outdir + wiki_code+'_global_users_namespace.csv'), 'w+') as data_file:
				...
				self.connect_database(wiki_code)
				input_file = csv.DictReader(open(self.dir + filename))
				for filerow in input_file:
					if filerow[wiki_code +'_wiki_user_id'] not in completed_users:						
						...
						if match_template:
							...
							if '-M' in proficiency_code:
								proficiency_code = proficiency_code.replace('-M', '-N')							
							if any(x in proficiency_code for x in a):
								...
								while attempt < 3 and not success:
									try:
										self.cur.execute(query_groupby_namespace, {'user_id':filerow[wiki_code +'_wiki_user_id']})
										logging.info("processing user " + filerow[wiki_code +'_wiki_user_id'])
										complete = False
										while not complete:
											...
											if not group_by_namespace_data:
												complete = True
												continue
											writer.writerow([filerow[wiki_code +'_wiki_user_id'], group_by_namespace_data['rev_count'], 
												group_by_namespace_data['total_rev_length'], group_by_namespace_data['page_namespace'], \
												filerow['home_wiki'], proficiency_code])
											completed_users.append(filerow[wiki_code +'_wiki_user_id'])
										success = True
									except Exception, e:
										attempt += 1
										traceback.print_exc()
										...
										if sqlEx[0] == 2006:
											...
											logging.error(sqlEx)
											self.connect_database(filename.split("_")[0])
								#else:
...
def main():
	...
	parser.add_argument('method', type=str, help='edit to get edits, namespace to get edit by namespace, titles to get titles')
	args = parser.parse_args()
	editPattern = EditPatternUserProficency()

	log = logging.getLogger()
	create_logger(log, args.method)	

	if args.method == 'edit':
		editPattern.edit_count_by_proficency()
	elif args.method == 'namespace':
		editPattern.user_namespace_contribution()
	elif args.method == 'titles':
